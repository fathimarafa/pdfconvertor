...
@Component({
  selector: 'app-subcontractor-work-orderapp-edit',
  templateUrl: './subcontractor-work-orderapp-edit.component.html',
  styleUrls: ['./subcontractor-work-orderapp-edit.component.css']
})
export class SubcontractorWorkOrderappEditComponent implements OnInit {
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<SubcontractorWorkOrderappEditComponent>,
  ...
  loadItemDetails() {
    if (this.isEdit) {
        const endpoint = `${WorkOrderApprovalMetadata.serviceEndPoint}List/${this.editData.id}`;
  ...
  ngOnInit(): void {
    this.tableColumns = WorkOrderApprovalMetadata.subContractorWorkOrderDetails.tableColumns;
  }
  ...
  defineModalForms() {
    this.modalForms = {
      issued: {
        ...
        model: this.editData,
        options: {},
        fields: WorkOrderApprovalMetadata.formFields
      },
      usage: {
        form: new FormGroup({}),
        model: {},
        options: {},
        fields: WorkOrderApprovalMetadata.subContractorWorkOrderDetails.formFields
      }
    }
  ...
  fetchSubcontractorIndent() {
    ...
    this.dataHandler.get<Indent[]>(`${'BuildExeHR/api/Indent'}/${1008}/${0}/${5}/${1}/${12}`)
  ...
  get httpRequest(): Observable<SubContractorWorkOrder> {
    ...
    if (this.isEdit) {
      return this.dataHandler.put<SubContractorWorkOrder>(WorkOrderApprovalMetadata.serviceEndPoint, [payload]);
    } else {
      payload.isDeleted = 0;
      return this.dataHandler.post<SubContractorWorkOrder>(WorkOrderApprovalMetadata.serviceEndPoint, [payload]);
    }
  }
...
// this.modalForm.indent.model.indentDetails = this.itemDatasource.data;
    this.dataHandler.put<SubContractorWorkOrder>(this.module.serviceEndPoint, [this.modalForms.issued.model]).subscribe((res) => {
