...
import { IndentForm, MaterialIndent } from '../definitions/material-indent-approval.definition';
...
@Component({
    selector: 'app-material-indent-approval-edit',
    templateUrl: './material-indent-approval-edit.component.html',
    styleUrls: ['./material-indent-approval-edit.component.css']
})
export class MaterialIndentApprovalEditComponent implements OnInit {

    isEdit: boolean;
    tableColumns = MaterialIndentApprovalMetadata.itemDetails.tableColumns;
    dataSource;
    modalForm: IndentForm;
    materialList: MaterialRegistration[];

    constructor(
        private dialogRef: MatDialogRef<MaterialIndentApprovalEditComponent>,
    ...
    defineForm() {
        this.modalForm = {
            indent: {
                ...
                model: this.editData,
                options: {},
                fields: MaterialIndentApprovalMetadata.formFields
            },
            itemDetails: {
                form: new FormGroup({}),
                model: {},
                options: {},
                fields: MaterialIndentApprovalMetadata.itemDetails.formFields
            }
        }
    ...
    loadItemDetails() {
        if (this.isEdit) {
            const endpoint = `${MaterialIndentApprovalMetadata.serviceEndPoint}List/${this.editData.id}`;
    ...
    onSaveBtnClick(nextLevel?: boolean) {
        if (this.modalForm.indent.form.valid) {
            if (this.isEditedFromApproval) {
                // if(this.modalForm.indent.model.approvalLevel===this.modalForm.nextLevel){//i
                this.openApproveDialog();
            // }//i
            } else {
                if (nextLevel) {
    ...
    get httpRequest(): Observable<MaterialIndent> {
        ...
        if (this.isEdit) {
            return this.dataHandler.put<MaterialIndent>(MaterialIndentApprovalMetadata.serviceEndPoint, [payload]);
        } else {
            payload.isDeleted = 0;
            return this.dataHandler.post<MaterialIndent>(MaterialIndentApprovalMetadata.serviceEndPoint, [payload]);
        }
    }
