...
import { PurchaseOrderDetail, MaterialPurchaseOrder } from '../definitions/material-purchase-order-approval.definition';
...
import { SelectIndentApprovalComponent } from './select-indent-approval/select-indent-approval.component';
...
@Component({
  selector: 'app-material-purchase-order-approval-edit',
  templateUrl: './material-purchase-order-approval-edit.component.html',
  styleUrls: ['./material-purchase-order-approval-edit.component.css']
})
export class MaterialPurchaseOrderApprovalEditComponent implements OnInit {
  ...
  constructor(
    private dialogRef: MatDialogRef<MaterialPurchaseOrderApprovalEditComponent>,
    ...
    this.editData = this.stateService.getState(MaterialPurchaseOrderApprovalMetadata.moduleId);
    if (this.editData) {
      this.isEdit = true;
    }
  ...
  defineModalForms() {
    this.modalForms = {
      purchaseOrder: {
        ...
        model: this.editData,
        options: {},
        fields: MaterialPurchaseOrderApprovalMetadata.formFields
      },
      purchaseOrderDetails: {
        form: new FormGroup({}),
        model: this.editData,
        options: {},
        fields: MaterialPurchaseOrderApprovalMetadata.purchaseOrderDetail.formFields
      }
    }
  ...
  populateItemDetailsTable() {
    this.tableColumns = MaterialPurchaseOrderApprovalMetadata.purchaseOrderDetail.tableColumns;
    if (this.isEdit) {
      const endpoint = `${MaterialPurchaseOrderApprovalMetadata.serviceEndPoint}List/${this.editData.id}`;
  ...
  get httpRequest(): Observable<MaterialPurchaseOrder> {
    ...
    payload.purchaseOrderDetail = this.dataSource.data;
    if (this.isEdit) {
        return this.dataHandler.put<MaterialPurchaseOrder>(MaterialPurchaseOrderApprovalMetadata.serviceEndPoint, [payload]);
    } else {
        payload.isDeleted = 0;
        return this.dataHandler.post<MaterialPurchaseOrder>(MaterialPurchaseOrderApprovalMetadata.serviceEndPoint, [payload]);
    }
}//indent
  ...
  onSelectIndentBtnClick() {
    ...
    if (this.indentList.length) {
      const dialogReference = this.dialog.open(SelectIndentApprovalComponent, { data: this.indentList });
  ...
  ngOnDestroy() {
    ...
    if (this.isEdit) {
      this.stateService.clear(MaterialPurchaseOrderApprovalMetadata.moduleId);
    }
  }
...
this.openApproveDialog();
// this.modalForms.indent.model.approvalLevel= this.modalForms.indent.model.approvalLevel-1;
this.modalForms.purchaseOrder.model.approvalLevel= this.modalForms.purchaseOrder.model.approvalLevel-1;
